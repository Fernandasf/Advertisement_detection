...
datadir_train = 'All_dataset/MSG/'
datadir_test = 'All_dataset/MSR/'

folds_train = ['Gtzan_5seg.txt']
folds_test_bases = [['Completa_5seg_test.txt'], ['NTT_5seg_test.txt'], ['TNT_5seg_test.txt']]

#folds_test_bases = [['Completa_5seg_test.txt'], ['TT_5seg_test.txt'], ['NTT_5seg_test.txt'], ['TNT_5seg_test.txt']]
...
f = open('Resultados_Speech/Speech_HN_MSG_MSR.txt', 'w')

for folds_test in folds_test_bases:

    print("\n ________________________________Base Testada: ", folds_test, "________________________________________________")
    ...
    for i in M:
        ...
        joblib.dump(model, "Features_Speech/model_HN_MSG_MSR"+str(i)+".pkl")
        joblib.dump(pred, "Features_Speech/y_pred_HN_MSG_MSR"+str(i)+".pkl")
        joblib.dump(ground_truth, "Features_Speech/y_test_HN_MSG_MSR"+str(i)+".pkl")
        joblib.dump(f_train, "Features_Speech/x_train_HN_MSG_MSR"+str(i)+".pkl")
        joblib.dump(y_train, "Features_Speech/y_train_HN_MSG_MSR"+str(i)+".pkl")

        print ("\n N. Train Classes: ", count_elements(genres_train))
        print ("N. Test Classes: ", count_elements(genres_test))

        ########### Etapa de Teste ##################
        print('\nBase de teste: ', folds_test, ' - Tempo: ', i)
        ...
        if folds_test == ['Completa_5seg_test.txt']:
            col1_array.append(round(F1, 2))
        if folds_test == ['NTT_5seg_test.txt']:
            col2_array.append(round(F1, 2))
        if folds_test == ['TNT_5seg_test.txt']:
...
np.savetxt('Resultados_Speech/Speech_HN_MSG_MSR.csv', (col0_array, col1_array, col2_array, col3_array), delimiter=',', fmt='%s')

...
def plot_confusion_matrix(cm, target_names, time, cmap=None, normalize=False, ext=''):
    ...
    if ext == '.png' or ext == 'NoNorm.png':
        plt.title('Confusion matrix - Handcrafted (N) - LMD - %i' %time)
    ...
    plt.savefig('Resultados/CM_HN_LMD'+str(time)+ext)
    #plt.show()
...
write_txt(f_train, g_train,  path='genres_train_LMD.txt')
write_txt(f_test, g_test, path='genres_test_LMD.txt')
...
M = [0.5, 1, 5, 10]
#
f = open('Resultados/Genre_HN_LMD.txt', 'w')
...
for i in M:
    ...
    joblib.dump(model, "Features/model_HN_LMD"+str(i)+".pkl")
    joblib.dump(pred, "Features/y_pred_HN_LMD"+str(i)+".pkl")
    joblib.dump(ground_truth, "Features/y_test_HN_LMD"+str(i)+".pkl")
    ...
    tn = ['axe', 'bachata', 'bolero', 'forro', 'gaucha', 'merengue', 'pagode', 'salsa', 'sertanejo', 'tango']
...
f.close()
np.savetxt('Resultados/Genre_HN_LMD.csv', (M_array, F1_array, F1_array_mean), delimiter=',', fmt='%s')
